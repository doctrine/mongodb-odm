...
class RequireIndexesTest extends BaseTest
{
    ...
    public function testGetFieldsInQueryWithSimpleEquals()
    {
        ...
        $query = $qb->getQuery();
        $this->assertEquals(['test'], $query->getFieldsInQuery());
    }
    ...
    public function testGetFieldsInQueryIgnoresWhereOperator()
    {
        ...
        $query = $qb->getQuery();
        $this->assertEquals(['username'], $query->getFieldsInQuery());
    }
    ...
    public function testGetFieldsInQueryWithOrAndIn()
    {
        $date = new \DateTime();
        $qb = $this->dm->createQueryBuilder('Doctrine\ODM\MongoDB\Tests\Functional\RequireIndexesDocument');
        $qb->addOr($qb->expr()->field('field1')->in([1]));
        $qb->addOr($qb->expr()->field('field2')->in([1]));
        $query = $qb->getQuery();
        $this->assertEquals(['field1', 'field2'], $query->getFieldsInQuery());
    }
    ...
    public function testGetFieldsInQueryWithReferences()
    {
        ...
        $query = $qb->getQuery();
        $this->assertEquals(['reference.$id', 'simpleReference'], $query->getFieldsInQuery());
    }
    ...
    public function testGetUnindexedFields()
    {
        $qb = $this->dm->createQueryBuilder('Doctrine\ODM\MongoDB\Tests\Functional\RequireIndexesDocument')
            ...
            ->field('indexed')->equals('test');
        $query = $qb->getQuery();
        $this->assertEquals(['embedOne.notIndexed', 'notIndexed'], $query->getUnindexedFields());
    }
}
