...
class ParserTest extends \Doctrine\ODM\MongoDB\Tests\BaseTest
{
    ...
    public function testSelectSlice()
    {
        ...
        $this->assertEquals(array('username', 'profile.firstName', 'comments' => array($this->escapeCommand('slice') => array(10, 20))), $query->debug('select'));

        $query = $this->parser->parse('find comments skip 10 Documents\User');
        $this->assertEquals(array('comments' => array($this->escapeCommand('slice') => array(10))), $query->debug('select'));

        $query = $this->parser->parse('find comments limit 10 Documents\User');
        $this->assertEquals(array('comments' => array($this->escapeCommand('slice') => array(0, 10))), $query->debug('select'));
    }

    public function testWhereMod()
    {
        $query = $this->parser->parse("find all Documents\User where a mod '[10, 1]'");
        $this->assertEquals(array('a' => array($this->escapeCommand('mod') => array(10, 1))), $query->debug('where'));

        $query = $this->parser->parse("find all Documents\User where not a mod '[10, 1]'");
        $this->assertEquals(array('a' => array($this->escapeCommand('not') => array($this->escapeCommand('mod') => array(10, 1)))), $query->debug('where'));
    }
    ...
    public function testWhereNot()
    {
        ...
        $this->assertEquals(array(
            'username' => array($this->escapeCommand('not') => 'jwage'),
            'count' => array($this->escapeCommand('not') => array($this->escapeCommand('gt') => 1)),
            'groups' => array($this->escapeCommand('not') => array($this->escapeCommand('in') => array(1, 2, 3)))
        ), $query->debug('where'));
    }
    ...
    public function testElemMatch()
    {
        ...
        $this->assertEquals(
            array(
                'accounts' => array(
                    $this->escapeCommand('elemMatch') => array(
                        'name' => 'test',
                        'type_name' => 'test'
                    )
                ),
    ...
    public function testComplexQuery()
    {
        ...
        $this->assertEquals(array(
            ...
            'field2' => 'changeme',
            'field3' => array(
              $this->escapeCommand('ne') => 'bob',
            ),
            'field4' => array(
              $this->escapeCommand('gt') => 1
            ),
            'field5' => array(
              $this->escapeCommand('gte') => 1
            ),
            'field6' => array(
              $this->escapeCommand('lt') => 5
            ),
            'field7' => array(
              $this->escapeCommand('gt') => 10
            ),
            'field8' => array(
              $this->escapeCommand('in') => array(1, 2, 3, 4)
            ),
            'field9' => array(
              $this->escapeCommand('nin') => array(5, 6)
            )
          ), $query->debug('where'));

        $this->assertEquals(array(
            $this->escapeCommand('set') => array(
              'field1' => 1,
            ),
            $this->escapeCommand('unset') => array(
              'field2' => 1,
            ),
            $this->escapeCommand('inc') => array(
              'field3' => 1,
            ),
            $this->escapeCommand('push') => array(
              'groups' => 1,
            ),
            $this->escapeCommand('pushAll') => array(
              'groups' => array(2, 3, 4),
            ),
            $this->escapeCommand('pull') => array(
              'groups' => 1,
            ),
            $this->escapeCommand('pullAll') => array(
              'groups' => array(2),
            ),
            $this->escapeCommand('pop') => array(
              'groups' => 1,
              'comments' => -1,
            ),
            $this->escapeCommand('addToSet') => array(
              'groups' => array(
                $this->escapeCommand('each') => array(4, 5, 6, 7),
              ),
            ),
          ), $query->debug('newObj'));
    }
    ...
    public function testPushAllComplexJsonValue()
    {
        ...
        $this->assertEquals(array($this->escapeCommand('pushAll') => array('groups' => array($group1, $group2))), $query->debug('newObj'));
    }
    ...
    public function testWhereInJsonValue()
    {
        ...
        $this->assertEquals(array('groups' => array($this->escapeCommand('in') => array(1, 2, 3))), $query->debug('where'));
    }

    public function testPushAllJsonValue()
    {
        $query = $this->parser->parse("update Documents\User pushAll groups = '[1, 2, 3]'");
        $this->assertEquals(array($this->escapeCommand('pushAll') => array('groups' => array(1, 2, 3))), $query->debug('newObj'));
    }
    ...
    public function testMultipleOperators()
    {
        ...
        $this->assertEquals(array($this->escapeCommand('set') => array('username' => 'jwage', 'password' => 'changeme'), $this->escapeCommand('inc') => array('count' => 1), $this->escapeCommand('push') => array('groups' => 1)), $query->debug('newObj'));
    }

    public function testNotEquals()
    {
        $query = $this->parser->parse("find all Documents\User where username != 'jwage'");
        $this->assertEquals(array('username' => array($this->escapeCommand('ne') => 'jwage')), $query->debug('where'));
    }
    ...
    public function testUpdate()
    {
        ...
        $this->assertEquals(array($this->escapeCommand('set') => array('username' => 'jwage', 'password' => 'changeme', 'groups' => array(1, 2, 3))), $query->debug('newObj'));
    }
    ...
    public function testIncrementOperator()
    {
        ...
        $this->assertEquals(array($this->escapeCommand('set') => array('username' => 'jwage'), $this->escapeCommand('inc') => array('count' => 1, 'views' => 2)), $query->debug('newObj'));
    }

    public function testUnsetOperator()
    {
        $query = $this->parser->parse("update Documents\User unset somefield, unset anotherfield");
        $this->assertEquals(array($this->escapeCommand('unset') => array('somefield' => 1, 'anotherfield' => 1)), $query->debug('newObj'));
    }

    public function testPushOperator()
    {
        $query = $this->parser->parse("update Documents\User push groups = :group", array(':group' => 1));
        $this->assertEquals(array($this->escapeCommand('push') => array('groups' => 1)), $query->debug('newObj'));
    }
    ...
    public function testPullOperator()
    {
        ...
        $this->assertEquals(array($this->escapeCommand('pull') => array('groups' => array(1, 2, 3))), $query->debug('newObj'));
    }
    ...
    public function testPullAllOperator()
    {
        ...
        $this->assertEquals(array($this->escapeCommand('pullAll') => array('groups' => array(1, 2, 3))), $query->debug('newObj'));
    }

    public function testPopFirstOperator()
    {
        $query = $this->parser->parse("update Documents\User popFirst groups, popFirst comments");
        $this->assertEquals(array($this->escapeCommand('pop') => array('groups' => 1, 'comments' => 1)), $query->debug('newObj'));
    }
    ...
    public function testAddToSet()
    {
        ...
        $this->assertEquals(array($this->escapeCommand('addToSet') => array('groups' => 1)), $query->debug('newObj'));
    }
    ...
    public function testAddManyToSet()
    {
        ...
        $this->assertEquals(array($this->escapeCommand('addToSet') => array('groups' => array($this->escapeCommand('each') => array(1, 2, 3)))), $query->debug('newObj'));
    }
    ...
    public function testGreaterThan()
    {
        ...
        $this->assertEquals(array('count' => array($this->escapeCommand('gt') => 1)), $query->debug('where'));
    }

    public function testGreaterThanOrEqualTo()
    {
        $query = $this->parser->parse('find username Documents\User where count >= 1');
        $this->assertEquals(array('count' => array($this->escapeCommand('gte') => 1)), $query->debug('where'));
    }

    public function testLessThan()
    {
        $query = $this->parser->parse('find username Documents\User where count < 1');
        $this->assertEquals(array('count' => array($this->escapeCommand('lt') => 1)), $query->debug('where'));
    }

    public function testLessThanOrEqualTo()
    {
        $query = $this->parser->parse('find username Documents\User where count <= 1');
        $this->assertEquals(array('count' => array($this->escapeCommand('lte') => 1)), $query->debug('where'));
    }
    ...
    public function testWhereNotIn()
    {
        ...
        $this->assertEquals(array('groups' => array($this->escapeCommand('nin') => array(1, 2, 3))), $query->debug('where'));
    }
    ...
    public function testWhereAll()
    {
        ...
        $this->assertEquals(array('groups' => array($this->escapeCommand('all') => array(1, 2, 3))), $query->debug('where'));
    }

    public function testWhereSize()
    {
        $query = $this->parser->parse('find all Documents\User where groups size 3');
        $this->assertEquals(array('groups' => array($this->escapeCommand('size') => 3)), $query->debug('where'));
    }

    public function testWhereExists()
    {
        $query = $this->parser->parse('find all Documents\User where groups exists true and comments exists false');
        $this->assertEquals(array('groups' => array($this->escapeCommand('exists') => true), 'comments' => array($this->escapeCommand('exists') => false)), $query->debug('where'));
    }

    public function testWhereType()
    {
        $query = $this->parser->parse('find all Documents\User where username type string');
        $this->assertEquals(array('username' => array($this->escapeCommand('type') => 2)), $query->debug('where'));
    }
}