...
want to check it from time to time during development.
    ...
    **CAUTION** Do not invoke ``flush`` after every change to a
...
database when ``DocumentManager#flush()`` is invoked.
    ...
    **CAUTION** Invoking the ``persist`` method on a document does NOT
...
.. code-block:: php
    ...
    **CAUTION** The \_id of a document is guaranteed to be available
...
document is unaffected by the ``remove`` operation.
    ...
    **CAUTION** Just like ``persist``, invoking ``remove`` on a
...
detached) and you want to modify and persist such a document.
    ...
    **NOTE** If you load some detached documents from a cache and you
